...
import com.beginnner.gharaana.Object.NitSignupRequest;
...
public class SignupRequestValidator {
    ...
    public static String nitSignupCheck(NitSignupRequest nitSignupRequest) {
        if (nitSignupRequest.phoneNo.length() != 10) {
            return "Enter PhoneNo of 10 digits";
        }

        for (int i = 0; i < 10; i++) {
            if (Character.isDigit(nitSignupRequest.phoneNo.charAt(i)) == false) {
                return "PhoneNo consists only digits";
            }
        }
        return null;
